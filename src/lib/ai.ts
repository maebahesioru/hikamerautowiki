...
export type ComposeInput = {
  ...
  tweets: TweetHit[];
  /** Wiki 内検索（list=search）の関連ページ。編集対象ページは含めない */
  wikiSearchHits?: Array<{ title: string; snippet?: string }>;
  /**
...
function buildUnifiedUserPrompt(input: ComposeInput, tweetBlock: string): string {
  ...
  const wikiBlock =
    wikiHits.length === 0
      ? "（Wiki 内検索で追加の関連ページはありません。）"
      : wikiHits
          .map((h, i) => {
            const snip = h.snippet?.trim()
              ? ` — ${h.snippet.trim()}`
              : "";
            return `${i + 1}. ${h.title}${snip}`;
          })
          .join("\n");
  ...
  lines.push(
    "【Wiki 内検索で得た関連ページ（list=search のタイトルとスニペット。編集対象ページは除いている）】",
    ...
    "出典としてツイートを参照する場合は、記事末尾に箇条書きで tweet id を明記する。",
    ...
    "- 事実の追加・変更は、可能な限り「現在の wikitext」「参考ツイート」「Wiki 内検索スニペット」のいずれかに裏付けがあるときに限る。裏付けがない主張は断定せず、推測である旨を書くか省略する。",
  );
...
export async function composeWikitextWithOpenAI(
  ...
  onProg?.(
    "OpenAI 互換 API に接続しています（巨大コンテキストでは初回トークンまで数分かかることがあります）…"
  );

  const finalize = (wikitext: string): string =>
    embedTwimgBracketLinksAsImg(
      fixMisusedFileNamespaceForExternalUrls(
        expandMediaRefsInWikitext(wikitext, tweetParts.mediaRefToUrl)
      )
    );
...
export async function suggestTweetSearchBundle(
  ...
  if (bootstrapTweets && bootstrapTweets.length > 0) {
    userLines.push(
      "",
      "【ページ名のみの検索で得た参考ツイート（検索クエリ案の材料。後続で他クエリも検索します）】",
      formatBootstrapTweetsForQueryPrompt(bootstrapTweets)
    );
  }
  ...
  const completion = useStream
    ...
    : await chatCompletionNonStream(messages, { jsonObject: true });
  streamOptions?.onTokenUsage?.(completion.usage);
  const content = completion.content;
...
async function buildFactCheckUserPrompt(
  ...
  return [
    ...
    '- items: 配列。各要素は { "claim": 検証した主張や文（日本語）, "verdict": "supported" | "weak" | "contradicted" | "unknown" のいずれか, "notes": 根拠・反証・保留理由（日本語）, "sources": 任意・文字列の配列（例: tweet:id, ページ名） }。',
    "verdict: 参考ツイート・Wiki スニペットで裏付けできるなら supported、弱いが矛盾しないなら weak、明確に矛盾するなら contradicted、材料不足なら unknown。",
    "ツイート・スニペットにない断定は unknown や weak に寄せ、推測を事実と書かない。",
  ].join("\n");
}
...
/**
 * 参考ツイート・Wiki スニペットを踏まえ、wikitext の事実性を JSON で返す（Wiki は更新しない）。
 */
export async function factCheckWithOpenAI(
  ...
  const messages: Array<{
  ...
  }> = [
    {
      role: "system",
      content:
        "あなたはファクトチェッカーです。与えられた wikitext（全文または抜粋）について、参考ツイートと Wiki 内検索スニペットだけを根拠に、主張ごとに検証する。JSON 以外は出力しない。推測で事実を断定しない。",
    },
